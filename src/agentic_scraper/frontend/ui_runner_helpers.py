...
async def display_progress(
    ...
    items = await run_worker_pool(
        ...
        config=WorkerPoolConfig(
            ...
            openai=st.session_state.get("openai_credentials"),
...
async def run_scraper_pipeline(
    ...
    if openai_credentials is None:
        message = "OpenAI credentials are required but were not found in settings."
...
def process_and_run(raw_input: str, config: PipelineConfig) -> tuple[list[ScrapedItem], int]:
    ...
    try:
        # Use openai_credentials directly from session state
        openai_credentials = st.session_state.get("openai_credentials")
        if not openai_credentials:
            st.error("OpenAI credentials are missing!")
            return [], 0

        # Update the config to include openai_credentials
        config.openai_credentials = openai_credentials
...
def maybe_run_pipeline(raw_input: str, controls: dict[str, Any]) -> tuple[list[ScrapedItem], int]:
    # Directly get openai_credentials from session_state
    openai_credentials = st.session_state.get("openai_credentials")
    if not openai_credentials:
        st.error("OpenAI credentials are missing!")
        return [], 0
    ...
    # Add openai_credentials to the config
    config.openai_credentials = openai_credentials
