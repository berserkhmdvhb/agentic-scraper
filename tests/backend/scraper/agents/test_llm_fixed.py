...
from agentic_scraper.backend.scraper.agents import llm_fixed as lf
...
class _FakeAsyncClient:
    """Matches the minimal surface used by llm_fixed.AsyncOpenAI."""
...
def _mk_request(
    ...
    *,
    ...
    # Provide a minimal, valid OpenAI config so llm_fixed can retrieve credentials.
...
async def test_extract_structured_data_success_no_screenshot(
    ...
    monkeypatch.setattr(lf, "AsyncOpenAI", _factory, raising=True)
    ...
    out = await lf.extract_structured_data(req, settings=settings)
...
async def test_extract_structured_data_calls_screenshot_when_enabled(
    ...
    async def _fake_capture(url: str, settings: Settings) -> str | None:
        _ = settings
        calls.append(url)
        # Use a safe, deterministic path (no temp path warning)
        return Path("snap.png").as_posix()

    monkeypatch.setattr(lf, "capture_optional_screenshot", _fake_capture, raising=True)
...
async def test_extract_structured_data_empty_content_returns_none(
    ...
    caplog.set_level("WARNING", logger="agentic_scraper.backend.scraper.agents.llm_fixed")
    ...
    # Make the check robust to minor punctuation/casing differences.
    assert "response was none" in err_text
...
async def test_extract_structured_data_handles_openai_exception(
    ...
    # Treat _BoomError as an OpenAIErrorT for the except clause in llm_fixed
    monkeypatch.setattr(lf, "OpenAIErrorT", _BoomError, raising=True)
    monkeypatch.setattr(lf, "AsyncOpenAI", _Client, raising=True)
    ...
    monkeypatch.setattr(lf, "handle_openai_exception", _spy_handle, raising=True)
