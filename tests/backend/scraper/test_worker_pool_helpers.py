...
def test_handle_success_item_appends_and_calls_callback() -> None:
...
def test_handle_success_item_callback_failure_is_guarded(
    ...
    def fake_warning(msg: str, *args: object, **_kwargs: object) -> None:
        seen.append(msg if not args else msg % args)
...
def test_handle_failure_warning_and_callback_guarded(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _Ctx:
        def __init__(self) -> None:
...
def test_call_progress_callback_happy_and_guarded(
    ...
    class _Ctx1:
        def __init__(self) -> None:
            ...
            self.on_progress = lambda _p, _t: None
...
async def test_place_ordered_result_places_by_first_index() -> None:
    ...
    ok1 = await helpers.place_ordered_result(
        context=cast("Any", ctx),
        url="u",
        item=item,
    )
    ok2 = await helpers.place_ordered_result(
        context=cast("Any", ctx),
        url="u",
        item=item,
    )
    assert ok1
    assert ok2
    assert [r.url if r else None for r in ctx.ordered_results] == [
        None,
        "https://u",
        "https://u",
    ]
